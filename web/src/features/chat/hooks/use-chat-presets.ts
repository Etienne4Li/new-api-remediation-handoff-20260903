...
function extractChats(status: SystemStatus | null): RawChatConfig {
  ...
  // Do not fall back to a legacy localStorage status object. Older versions
  // persisted complete chat templates (including `{key}` placeholders), and
  // reviving that cache would bypass the backend's public projection.
