...
describe('sidebar role visibility', () => {
  test('exposes quota recharge alongside the wallet entry', () => {
    const { result } = renderHook(() => useSidebarData())
    const personal = result.current.navGroups.find(
      (group) => group.id === 'personal'
    )
    const quotaRecharge = personal?.items.find(
      (item) => 'url' in item && item.url === '/quota-recharge'
    )

    expect(quotaRecharge?.title).toBe('Quota Recharge')
  })
