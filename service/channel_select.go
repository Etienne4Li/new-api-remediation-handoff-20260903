...
func (p *RetryParam) ExcludeChannel(channelID int, isMultiKey bool) {
	// A multi-key channel selects another enabled key on its next attempt. A
	// single failed key must not remove the whole channel from this request.
	if channelID <= 0 || isMultiKey {
		return
	}
...
func CacheGetRandomSatisfiedChannel(param *RetryParam) (*model.Channel, string, error) {
	...
	if param.TokenGroup == "auto" {
		...
		for i := startGroupIndex; i < len(autoGroups); i++ {
			...
			channel, err = model.GetRandomSatisfiedChannel(autoGroup, param.ModelName, priorityRetry, param.RequestPath, param.excludedChannelIDs)
			if err != nil {
