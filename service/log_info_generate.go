...
func AppendRelayLogAdminInfo(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, other *model.LogOther) {
	...
	appendResponsesInputItemIDNormalizationAdminInfo(ctx, other)
}

// appendResponsesInputItemIDNormalizationAdminInfo bridges the map-based helper
// in relay/common onto the scoped LogOther introduced upstream.
func appendResponsesInputItemIDNormalizationAdminInfo(ctx *gin.Context, other *model.LogOther) {
	if ctx == nil || other == nil {
		return
	}
	extra := make(map[string]interface{})
	relaycommon.AppendResponsesInputItemIDNormalizationAdminInfo(ctx, extra)
