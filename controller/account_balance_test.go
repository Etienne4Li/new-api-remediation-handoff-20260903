...
func TestGetAccountBalanceUsesUserQuotaThroughReadOnlyTokenAuth(t *testing.T) {
	...
	tokens := []model.Token{
		{UserId: user.Id, Key: "balancelimited", Name: "limited", Status: common.TokenStatusEnabled, ExpiredTime: -1, RemainQuota: 7},
		{UserId: user.Id, Key: "balanceunlimited", Name: "unlimited", Status: common.TokenStatusEnabled, ExpiredTime: -1, RemainQuota: 999_999_999, UnlimitedQuota: true},
		{UserId: user.Id, Key: "balanceexhausted", Name: "exhausted", Status: common.TokenStatusExhausted, ExpiredTime: -1},
		{UserId: user.Id, Key: "balancedisabled", Name: "disabled", Status: common.TokenStatusDisabled, ExpiredTime: -1},
		{UserId: user.Id, Key: "balanceexpiredstatus", Name: "expired status", Status: common.TokenStatusExpired, ExpiredTime: -1},
		{UserId: user.Id, Key: "balanceunknownstatus", Name: "unknown status", Status: 99, ExpiredTime: -1},
		{UserId: user.Id, Key: "balanceexpiredtime", Name: "expired time", Status: common.TokenStatusEnabled, ExpiredTime: time.Now().Add(-time.Hour).Unix()},
		{UserId: user.Id, Key: "balance-hyphenated", Name: "hyphenated", Status: common.TokenStatusEnabled, ExpiredTime: -1},
		{UserId: user.Id, Key: "balance", Name: "prefix collision", Status: common.TokenStatusDisabled, ExpiredTime: -1},
	}
	allowedIP := "203.0.113.10"
	tokens = append(tokens, model.Token{UserId: user.Id, Key: "balanceiplimited", Name: "ip limited", Status: common.TokenStatusEnabled, ExpiredTime: -1, AllowIps: &allowedIP})
	...
	for _, key := range []string{"balancelimited", "balanceunlimited", "balanceexhausted"} {
	...
	authTests := []struct {
		...
		{name: "expired status", key: "balanceexpiredstatus", wantStatus: http.StatusUnauthorized},
		{name: "expired timestamp", key: "balanceexpiredtime", wantStatus: http.StatusUnauthorized},
		{name: "IP restriction", key: "balanceiplimited", wantStatus: http.StatusForbidden},
	...
	// The parser is shared with normal relay authentication: mixed-case Bearer
	// and repeated whitespace must not accidentally bypass the status checks.
	mixedCase := httptest.NewRequest(http.MethodGet, "/api/usage/balance/", nil)
	mixedCase.Header.Set("Authorization", "  bEaReR   sk-balancelimited  ")
	mixedRecorder := httptest.NewRecorder()
	router.ServeHTTP(mixedRecorder, mixedCase)
	assert.Equal(t, http.StatusOK, mixedRecorder.Code)
	assert.Contains(t, mixedRecorder.Header().Get("Cache-Control"), "no-store")

	unknown := performAccountBalanceRequest(router, "balanceunknownstatus")
	assert.Equal(t, http.StatusUnauthorized, unknown.Code)
	hyphenated := performAccountBalanceRequest(router, "balance-hyphenated")
	assert.Equal(t, http.StatusOK, hyphenated.Code)
	// A read-only credential must not silently truncate at a hyphen and select
	// another token row.
	prefixCollision := performAccountBalanceRequest(router, "balance-123")
	assert.Equal(t, http.StatusUnauthorized, prefixCollision.Code)
}
