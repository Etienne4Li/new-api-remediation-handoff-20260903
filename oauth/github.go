...
func (p *GitHubProvider) IsEnabled() bool {
	return common.GetSecurityRuntimeConfig().GitHubOAuthEnabled
}
...
func (p *GitHubProvider) ExchangeTokenWithVerifier(ctx context.Context, code, codeVerifier string, c *gin.Context) (*OAuthToken, error) {
	if code == "" {
		return nil, NewOAuthError(i18n.MsgOAuthInvalidCode, nil)
	}

	logger.LogDebug(ctx, "[OAuth-GitHub] ExchangeToken: code_meta=%s", common.SensitiveLogMeta(code))
	securityConfig := common.GetSecurityRuntimeConfig()

	values := map[string]string{
		"client_id":     securityConfig.GitHubClientID,
		"client_secret": securityConfig.GitHubClientSecret,
		"code":          code,
	}
	if redirectURI := RedirectURIFromContext(ctx); redirectURI != "" {
		// GitHub accepts redirect_uri at the token endpoint and requires it to
		// match the URI used by the authorization request when one is supplied.
		// Keep it optional for legacy callers that did not persist a flow URI.
	...
	if err != nil {
		logger.LogError(ctx, fmt.Sprintf("[OAuth-GitHub] ExchangeToken error_meta=%s", common.SensitiveLogMeta(err.Error())))
		return nil, NewOAuthErrorWithRaw(i18n.MsgOAuthConnectFailed, map[string]any{"Provider": "GitHub"}, common.SensitiveLogMeta(err.Error()))
	}
...
func (p *GitHubProvider) GetUserInfo(ctx context.Context, token *OAuthToken) (*OAuthUser, error) {
	...
	res, err := client.Do(req)
	if err != nil {
		logger.LogError(ctx, fmt.Sprintf("[OAuth-GitHub] GetUserInfo error_meta=%s", common.SensitiveLogMeta(err.Error())))
		return nil, NewOAuthErrorWithRaw(i18n.MsgOAuthConnectFailed, map[string]any{"Provider": "GitHub"}, common.SensitiveLogMeta(err.Error()))
	}
	defer res.Body.Close()
	...
	if res.StatusCode != http.StatusOK {
		body, readErr := readOAuthResponseBody(res)
		if readErr != nil {
			logger.LogError(ctx, fmt.Sprintf("[OAuth-GitHub] GetUserInfo failed: status=%d, body_read_error=%s", res.StatusCode, common.SensitiveLogMeta(readErr.Error())))
		} else {
			logger.LogError(ctx, fmt.Sprintf("[OAuth-GitHub] GetUserInfo failed: status=%d, body_meta=%s", res.StatusCode, common.SensitiveLogBody(body)))
		}
